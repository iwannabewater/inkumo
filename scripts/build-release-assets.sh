...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/inkumo-release-build.XXXXXX")"
...
for pass in 1 2; do
  if ! (
    ...
    xelatex -interaction=nonstopmode -halt-on-error -file-line-error \
      -jobname=resume-avatar "$avatar_input"
  ) >"$LOG_DIR/resume-avatar-pass-$pass.txt" 2>&1; then
    cat "$LOG_DIR/resume-avatar-pass-$pass.txt" >&2
    inkumo_fail "avatar release PDF failed on XeLaTeX pass $pass"
  fi
done

bash "$ROOT_DIR/scripts/check-rendered-pdf.sh" resume-avatar

text_pages="$(pdfinfo "$ROOT_DIR/resume.pdf" | awk '/^Pages:/ {print $2}')"
avatar_pages="$(pdfinfo "$ROOT_DIR/resume-avatar.pdf" | awk '/^Pages:/ {print $2}')"
[[ "$text_pages" = "$avatar_pages" ]] \
  || inkumo_fail "release PDFs must have the same page count"

if cmp -s "$ROOT_DIR/resume.pdf" "$ROOT_DIR/resume-avatar.pdf"; then
  inkumo_fail "avatar release PDF is identical to the text-only PDF"
fi

mkdir -p "$OUTPUT_DIR"
cp "$ROOT_DIR/resume.pdf" "$OUTPUT_DIR/resume.pdf"
cp "$ROOT_DIR/resume-avatar.pdf" "$OUTPUT_DIR/resume-avatar.pdf"
