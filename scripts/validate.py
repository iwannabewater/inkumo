...
def write_icon_audit(registry_path, output_path):
    ...
    lines = [
        ...
        r"\newcommand{\AssertAdaptiveIconSlot}[1]{%",
        r"  \setbox0=\hbox{\PIcon{#1}}%",
        r"  \ifdim\wd0<8.8pt",
        r"    \errmessage{Icon #1 is narrower than the minimum slot width}%",
        r"  \fi",
        r"  \ifdim\wd0>11pt",
        r"    \errmessage{Icon #1 exceeds the maximum slot width}%",
        r"  \fi",
        ...
        r"  \fi",
        r"}",
        r"\newcommand{\AssertIconWidthRange}[3]{%",
        r"  \setbox0=\hbox{\PIcon{#1}}%",
        r"  \ifdim\wd0<#2",
        r"    \errmessage{Icon #1 is narrower than its shape class permits}%",
        r"  \fi",
        r"  \ifdim\wd0>#3",
        r"    \errmessage{Icon #1 is wider than its shape class permits}%",
        r"  \fi",
        r"}",
        r"\makeatletter",
        r"\newcommand{\AssertIconVisualBounds}[1]{%",
        r"  \begingroup",
        r"  \fontsize{8.8pt}{8.8pt}\selectfont",
        r"  \inkumo@fiticon{\csname inkumo@picon@#1\endcsname}%",
        r"  \dimen0=\ht\inkumo@iconfitbox",
        r"  \advance\dimen0 by \dp\inkumo@iconfitbox",
        r"  \ifdim\wd\inkumo@iconfitbox>11pt",
        r"    \errmessage{Icon #1 exceeds the visual width bound}%",
        r"  \fi",
        r"  \ifdim\dimen0>8.61pt",
        r"    \errmessage{Icon #1 exceeds the visual height bound}%",
        r"  \fi",
        r"  \ifdim\wd\inkumo@iconfitbox>0pt\else",
        r"    \errmessage{Icon #1 has no visible width}%",
        r"  \fi",
        r"  \ifdim\dimen0>0pt\else",
        r"    \errmessage{Icon #1 has no visible height}%",
        r"  \fi",
        r"  \endgroup",
        r"}",
        r"\newcommand{\AssertIconVisualRange}[5]{%",
        r"  \begingroup",
        r"  \fontsize{8.8pt}{8.8pt}\selectfont",
        r"  \inkumo@fiticon{\csname inkumo@picon@#1\endcsname}%",
        r"  \dimen0=\ht\inkumo@iconfitbox",
        r"  \advance\dimen0 by \dp\inkumo@iconfitbox",
        r"  \ifdim\wd\inkumo@iconfitbox<#2",
        r"    \errmessage{Icon #1 visible width is below its shape range}%",
        r"  \fi",
        r"  \ifdim\wd\inkumo@iconfitbox>#3",
        r"    \errmessage{Icon #1 visible width exceeds its shape range}%",
        r"  \fi",
        r"  \ifdim\dimen0<#4",
        r"    \errmessage{Icon #1 visible height is below its shape range}%",
        r"  \fi",
        r"  \ifdim\dimen0>#5",
        r"    \errmessage{Icon #1 visible height exceeds its shape range}%",
        r"  \fi",
        r"  \endgroup",
        r"}",
        r"\makeatother",
    ...
    lines.extend(r"\AssertAdaptiveIconSlot{%s}" % key for key in keys)
    lines.extend(r"\AssertIconVisualBounds{%s}" % key for key in keys)
    lines.extend(
        [
            r"\AssertIconWidthRange{x}{8.7pt}{8.9pt}",
            r"\AssertIconWidthRange{zhihu}{8.7pt}{8.9pt}",
            r"\AssertIconWidthRange{education}{10.8pt}{11pt}",
            r"\AssertIconWidthRange{major}{9.8pt}{10.1pt}",
            r"\AssertIconWidthRange{profession}{10.1pt}{10.5pt}",
            r"\AssertIconVisualRange{x}{8.1pt}{8.3pt}{8.1pt}{8.3pt}",
            r"\AssertIconVisualRange{zhihu}{8.1pt}{8.3pt}{8.1pt}{8.3pt}",
            r"\AssertIconVisualRange{education}{10.9pt}{11pt}{6.5pt}{6.7pt}",
            r"\AssertIconVisualRange{major}{9.8pt}{10pt}{7.7pt}{7.9pt}",
            r"\AssertIconVisualRange{profession}{10.1pt}{10.4pt}{8.1pt}{8.3pt}",
            r"\AssertIconVisualRange{location}{6.3pt}{6.5pt}{8.5pt}{8.61pt}",
        ]
    )
